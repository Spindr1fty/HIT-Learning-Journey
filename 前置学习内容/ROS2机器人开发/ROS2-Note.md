...
Humble Hawksbill  发布日期:2022.5.23    停止维护日期:2027.5
（本课程配套软件）

ROS 2最核心的地方是通信，ROS 2机器人开发特色为
一.提供四大核心通信机制
1.话题(Topic)：基于发布-订阅模式的通信方式，允许节点之间异步交换数据
2.服务(Service):同步通信方式，客户端发送请求，服务端处理并返回结果
3.参数(Parameter):用于机器人参数的设置和读取
4.动作(Action):支持复杂行为的通信方式，服务端可以反馈处理进度，客户端可以取消请求
二.拥有各种可视化调试工具
三.拥有丰富的建模与运动学工具
四.拥有强大的开源社区和应用框架，例：
